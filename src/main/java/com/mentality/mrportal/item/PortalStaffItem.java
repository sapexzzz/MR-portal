...
public class PortalStaffItem extends Item {
	...
	public void appendHoverText(ItemStack stack, Level level, List<Component> tooltipComponents, TooltipFlag isAdvanced) {
		tooltipComponents.add(ModTranslation.get(this.infinite ? "item.mr_portal.infinite_portal_staff.tooltip" : "item.mr_portal.portal_staff.tooltip").withStyle(ChatFormatting.GRAY));
		if (this.infinite) {
			tooltipComponents.add(ModTranslation.get("item.mr_portal.portal_staff.cooldown_disabled").withStyle(ChatFormatting.AQUA));
			return;
		}

		CompoundTag tag = stack.getTag();
		if (tag == null || tag.getBoolean(TOOLTIP_READY_KEY)) {
			tooltipComponents.add(ModTranslation.get("item.mr_portal.portal_staff.cooldown_ready").withStyle(ChatFormatting.GREEN));
			return;
		}

		int secondsLeft = Math.max(1, tag.getInt(TOOLTIP_SECONDS_KEY));
		tooltipComponents.add(ModTranslation.get("item.mr_portal.portal_staff.cooldown_seconds", secondsLeft).withStyle(ChatFormatting.GOLD));
	}
}