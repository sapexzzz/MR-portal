...
public final class MRPortalConfigScreen {
	...
	public static Screen create(Screen parent) {
		...
		portal.addEntry(entry.startIntField(ModTranslation.get("config.mr_portal.quick_favorite_portal_delay"), config.quickFavoritePortalDelayTicks)
			.setDefaultValue(defaults.quickFavoritePortalDelayTicks)
			.setMin(0)
			.setSaveConsumer(val -> config.quickFavoritePortalDelayTicks = val)
			.setTooltip(ModTranslation.get("config.mr_portal.quick_favorite_portal_delay.tooltip"))
			.build());

		portal.addEntry(entry.startDoubleField(ModTranslation.get("config.mr_portal.portal_exit_behind_distance"), config.portalExitBehindDistance)
			.setDefaultValue(defaults.portalExitBehindDistance)
			.setMin(0.0D)
			.setMax(3.0D)
			.setSaveConsumer(val -> config.portalExitBehindDistance = val)
			.setTooltip(ModTranslation.get("config.mr_portal.portal_exit_behind_distance.tooltip"))
			.build());
