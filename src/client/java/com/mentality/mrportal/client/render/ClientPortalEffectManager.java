...
public final class ClientPortalEffectManager {
	...
	public static void addEffect(UUID sessionId, boolean source, ResourceKey<Level> dimension, double x, double y, double z, float yaw, int openDelay, int lifetime, float scale, float animSpeed) {
		removeMatchingPreview(dimension, x, y, z);
		PortalEffect effect = new PortalEffect(dimension, new Vec3(x, y, z), yaw, openDelay, lifetime, scale, animSpeed);
	...
	private static void tick(Minecraft client) {
		...
		while (iterator.hasNext()) {
			...
			entry.getValue().removeIf(effect -> {
				...
				if (!effect.playedOpenSound && effect.age >= sparkDuration && client.level.dimension().equals(effect.dimension)) {
	...
	private static final class PortalEffect {
		...
		private PortalEffect(ResourceKey<Level> dimension, Vec3 position, float yaw, int openDelay, int lifetime, float scale, float animSpeed) {
