...
public final class ClientPortalEffectManager {
	...
	private static final Map<UUID, PreviewSpark> remoteSparks = new HashMap<>();
	...
	public static void addEffect(UUID sessionId, boolean source, ResourceKey<Level> dimension, double x, double y, double z, float yaw, int openDelay, int lifetime, float scale, float animSpeed, boolean playChime, boolean playOpenSound) {
		removeMatchingPreview(dimension, x, y, z);
		PortalEffect effect = new PortalEffect(dimension, new Vec3(x, y, z), yaw, openDelay, lifetime, scale, animSpeed, playOpenSound);
		EFFECTS.computeIfAbsent(sessionId, ignored -> new ArrayList<>()).add(effect);
		Minecraft minecraft = Minecraft.getInstance();
		if (playChime && minecraft.level != null && minecraft.level.dimension().equals(dimension)) {
	...
	public static void showRemoteSpark(UUID playerId, ResourceKey<Level> dimension, double x, double y, double z, float yaw, float scale) {
		remoteSparks.put(playerId, new PreviewSpark(dimension, new Vec3(x, y, z), yaw, scale));
	}

	public static void hideRemoteSpark(UUID playerId) {
		remoteSparks.remove(playerId);
	}
	...
	private static void tick(Minecraft client) {
		...
		for (PreviewSpark remote : remoteSparks.values()) {
			if (client.level.dimension().equals(remote.dimension)) {
				remote.age++;
				spawnSparkParticles(client, remote.position, remote.age, true);
			}
		}
		...
		while (iterator.hasNext()) {
			...
			entry.getValue().removeIf(effect -> {
				effect.age++;
				int sparkDuration = Math.max(1, (int) (SPARK_DURATION / effect.animSpeed));
				int closeDuration = Math.max(1, (int) (CLOSE_DURATION / effect.animSpeed));
				if (effect.playOpenSound && !effect.playedOpenSound && effect.age >= sparkDuration && client.level.dimension().equals(effect.dimension)) {
				...
				if (client.level.dimension().equals(effect.dimension)) {
					if (effect.age <= sparkDuration) {
						spawnSparkParticles(client, effect.position, effect.age, false);
					}
					if (effect.closing && effect.closeTick >= closeDuration / 2) {
				...
				return effect.age > effect.lifetime || effect.closeTick > closeDuration;
			});
	...
	private static void render(PoseStack poseStack, Vec3 cameraPos) {
		...
		for (PreviewSpark remote : remoteSparks.values()) {
			if (minecraft.level != null && minecraft.level.dimension().equals(remote.dimension)) {
				renderDisk(poseStack, cameraPos, remote.position, remote.yaw, 0.08F * remote.scale, 0.12F * remote.scale, 1.0F, 1.0F, 1.0F, 0.95F);
			}
		}
		...
		for (List<PortalEffect> effects : EFFECTS.values()) {
			for (PortalEffect effect : effects) {
				...
				int sparkDuration = Math.max(1, (int) (SPARK_DURATION / effect.animSpeed));
				int closeDuration = Math.max(1, (int) (CLOSE_DURATION / effect.animSpeed));
				float closeProgress = effect.closing ? Mth.clamp(effect.closeTick / (float) closeDuration, 0.0F, 1.0F) : 0.0F;
				float openProgress = Mth.clamp((effect.age - sparkDuration) / (float) Math.max(1, effect.openDelay - sparkDuration), 0.0F, 1.0F);
				...
				float openingSparkAlpha = effect.age <= sparkDuration ? 1.0F - (effect.age / (float) sparkDuration) : 0.0F;
	...
	private static final class PortalEffect {
		...
		private PortalEffect(ResourceKey<Level> dimension, Vec3 position, float yaw, int openDelay, int lifetime, float scale, float animSpeed, boolean playOpenSound) {
