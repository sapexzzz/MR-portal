...
public final class MRPortalClientNetworking {
	...
	public static void register() {
		...
		ClientPlayNetworking.registerGlobalReceiver(MRPortalNetworking.PORTAL_CREATE_S2C, (client, handler, buffer, responseSender) -> {
			...
			boolean playChime = buffer.readBoolean();
			boolean playOpenSound = buffer.readBoolean();
			client.execute(() -> ClientPortalEffectManager.addEffect(sessionId, source, dimension, x, y, z, yaw, openDelay, lifetime, scale, animSpeed, playChime, playOpenSound));
		});
	...
	public static void sendQuickTeleportByKeybind() {
		FriendlyByteBuf buffer = PacketByteBufs.create();
		ClientPlayNetworking.send(MRPortalNetworking.QUICK_TELEPORT_BY_KEYBIND_C2S, buffer);
	}

	public static void sendSetFavoriteWaypoint(UUID waypointId, boolean favorite) {
		FriendlyByteBuf buffer = PacketByteBufs.create();
		buffer.writeUUID(waypointId);
		buffer.writeBoolean(favorite);
		ClientPlayNetworking.send(MRPortalNetworking.SET_FAVORITE_WAYPOINT_C2S, buffer);
	}
}