...
public class MRPortalClient implements ClientModInitializer {
	...
	private static KeyMapping quickFavoriteTeleportKey;
	...
	public void onInitializeClient() {
		...
		quickFavoriteTeleportKey = KeyBindingHelper.registerKeyBinding(new KeyMapping(
			"key.mr_portal.quick_favorite_teleport",
			InputConstants.Type.KEYSYM,
		...
		ClientTickEvents.END_CLIENT_TICK.register(client -> {
			...
			while (quickFavoriteTeleportKey.consumeClick()) {
				if (client.player != null && client.screen == null) {
					MRPortalClientNetworking.sendQuickTeleportByKeybind();
				}
			}
		});
	}
